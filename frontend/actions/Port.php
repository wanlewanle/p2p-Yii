...
//                  self::phongLog($phone,$code,Port::CONFIRM,'发送成功，验证码错误');
    ...
    public static function sendSms2($mobile,$verify,$verify_time,$templateid){
        ...
        if($templateid == '1'){
            $template = '【活琪王】注册校验码：'.$verify.',有效时间'.$verify_time.'分钟，感谢注册活琪王，如需帮助请致电：4006 985 185.';
        }elseif($templateid == '2'){
            $template = '【活琪王】尊敬的投资者，您已成功购买'.$verify.'元活琪王理财，即日起开始计息！';
        }elseif($templateid == '3'){
            $template = '【活琪王】尊敬的投资者，您于'.$send_time.' 发起一笔提现申请，请求已处理，资金会在24小时内到账，敬请留意';
        }elseif($templateid == '4'){
            $template = '【活琪王】尊敬的投资者，您于'.$send_time.'进行了密码重置操作，短信校验码为'.$verify.',有效时间'.$verify_time.'分钟，如非本人操作，请忽略';
        }elseif($templateid == '5'){
            $template = '【活琪王】尊敬的投资者，您于'.$send_time.'进行了修改密码操作，短信校验码为'.$verify.',有效时间'.$verify_time.'分钟，如非本人操作，请忽略';
        }elseif($templateid == '6'){
            $template = '';
        }
        //667857
